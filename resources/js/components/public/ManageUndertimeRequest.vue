            ...
            <form method="post">
                ...
                <div class="col-md-8">
                            <div class="form-group-limit">
                                        <Datepicker :value="date_undertime" @selected="selectDateUndertime" wrapper-class="mdb-form-field" input-class="form-field__input datePicker" :typeable="false" :format="'MM/dd/yyyy'">
                ...
                <div class="modal-footer">
                    <input type="submit" class="btn btn-primary" value="Submit" @click.prevent="addUnderTime" :disabled="disabledIfNoApprover || isDisable || !isFormValid" v-if="!undertimeID && $parent.$data.forapprover != 'approval'">
                    <input type="submit" class="btn btn-primary" value="Update" @click.prevent="updateUnderTime" :disabled="isDisable || !isFormValid" v-if="undertimeID && $parent.$data.forapprover != 'approval' && !$parent.disabledinput">
                    <input type="submit" class="btn btn-primary" value="Delete" @click.prevent="deleteUnderTime" :disabled="isDisable" v-if="undertimeID && $parent.$data.forapprover != 'approval' && !$parent.disabledinput ">
...
export default {
    ...
    data() {
        ...
        date_undertime: moment(new Date()).format('MM/DD/YYYY'),
    ...
    methods:{
        selectDateUndertime(val){
            this.date_undertime = moment(val).format('MM/DD/YYYY');
        },
        addUnderTime(){
            if(this.isFormValid){
                this.isDisable = true;
        ...
        closeModal(){
            ...
            Object.keys(obj).forEach((key)=>{
                ...
                if(key != 'datefiled' && key != 'starttime' && key != 'endtime' && key != 'date_undertime'){
                    this.$data[key] =  '';
                }
                if(key=='isDisable'){
                    this.$data[key] = false;
                }
                if(key == 'datefiled' || key == 'date_undertime'){
                    this.datefiled = moment(new Date()).format('MM/DD/YYYY');
                    this.date_undertime = moment(new Date()).format('MM/DD/YYYY');
                }
            });
            $("#myModal").modal("hide");
        },
