            ...
            <div class="col-lg-12">
                <div class="dflex">
                    <div class="file-upload mb-3">
                        <input type="file" class="custom-file-input" id="file-policy" @change="uploadXLS" name="images[]" style="display: none;" multiple>
                        <label class="btn btn-primary" for="file-policy">Choose file</label>
                    </div>
...
export default {
    ...
    methods:{
        ...
        async validate(e){
            ...
            if(document.querySelector('input#file-policy[type=file]').value != ''){
                let policy_id = this.policy.id;
                const validImageTypes = ["application/pdf"];
                let count = 0;

                // let files    = document.querySelector('input#file-policy[type=file]').files[0];
                let files    = document.querySelector('input#file-policy[type=file]').files;
                ...
                // reset table and clear directory
                // if(totalFiles>0){
                //     let cleared = await this.clearPolicyTable();
                //     if(!cleared){
                //         alert('Server Error');
                //         return;
                //     }
                ...
                for(var key in targetFiles){
                    ...
                    if(targetFiles.length > count){
                        ...
                        if (!validImageTypes.includes(type)) {
                            ...
                            document.querySelector('input#file-policy[type=file]').value = '';
                            alert('Invalid pdf type');
                            return false;
                        }
                    }
                    await formData.append('attachment[]', targetFiles[key]);
                    await formData.append('policy_id', policy_id);

                    if((count + 1) % 10 == 0){
                        this.postPolicy(formData, false);
                        formData = new FormData();

                    }
                    if(iterators < count+1){
                        await remaining_formData.append('attachment[]', targetFiles[key]);
                        await remaining_formData.append('policy_id', policy_id);
                    }
                ...
                this.postPolicy(remaining_formData, true);
                return false;
                // return await formData;
            }
        },
        async clearPolicyTable(){
            let policy_id = this.policy.id;
            try{
                await axios.post('api/add-policy', {clear: true, policy_id: policy_id});
                return true;
            }
            catch(err){
                return false;
        ...
        postPolicy(files, isDone = false){


            axios.post('api/add-policy', files)
            .then(res=>{
                if(isDone){
