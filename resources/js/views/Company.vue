    ...
    methods:{
       ...
       setUpdate(data){
        ...
        delRow(id=''){
            ...
            if (result) {
                axios.post('api/delcompany',{
                    compID: id
                }).then((res)=>{
                    if(res.data.hasOwnProperty('error')){
                        alert('Cannot delete this company. \nThere are '+res.data.error+' users from this company.\nmake sure to assign them first to another company.');
                    }else{
                        let row = this.$data.rows;
                        let data = row.filter((obj)=>obj.compID != id);
                        this.rows = data;
                    }
    ...
    mounted() {
        ...
        axios.get('api/getcompany').then((response)=>{
            ...
            $("#managecomp tbody").on('click', 'button.del-btn', function() {
                ...
                self.delRow(dataforedit.compID);
                remove_emp = true;
            });
