...
export default {
    ...
    methods:{
        ...
        delRow(id='', deptname = ''){
            ...
            if (result) {
                axios.post('api/deldept',{
                    deptID: id,
                    deptname: deptname,
                }).then((res)=>{
                    if(res.data.hasOwnProperty('error')){
                        alert('Cannot delete this department. \nThere are '+res.data.error+' users from this department.\nmake sure to assign them first to another department.');
                    }else{
                        let row = this.$data.rows;
                        let data = row.filter((obj)=>obj.deptID != id);
                        this.rows = data;
                    }
    ...
    mounted() {
        ...
        axios.get('api/getdept').then((response)=>{
            this.loader = false;
            ...
            $("#managedept tbody").on('click', 'button.del-btn', function() {
                let tr = $(this).closest('tr');
                let row = table.row( tr );
                let dataforedit = row.data();
                // self.delRow(dataforedit.deptID);
                // console.log(dataforedit.deptID);
                self.delRow(dataforedit.deptID, dataforedit.deptname);
                remove_emp = true;
            });
