...
export default {
    ...
    methods:{
        ...
        updateRow(val)
        {
            ...
            row.forEach((item, index)=>{
                ...
                if(item.supID == val.supID)
                {
                    ...
                    data['witnesses'] = item.witnesses? // to avoid null + witness
                                        item.witnesses +', '+val.witnesses
                                        : ', '+val.witnesses;
        ...
        setUpdate(data){
            ...
            if(data.witnesses && (data.witnesses).includes(this.$root.$data.userinfo.fullname)){
                this.witness_approval = false;
            }
    ...
    mounted(){
        ...
        axios.get('api/get-supplementary-approvalwitnes').then((response)=>{
            this.loader = false;
            ...
            "rowCallback": (row, data, index) =>{
                var cellValue = data["witnesses"];
                    if(cellValue && cellValue.includes(this.$root.$data.userinfo.fullname)){
