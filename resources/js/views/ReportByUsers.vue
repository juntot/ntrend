...
export default {
    ...
    methods:{
        manageUpdateReport(empid = null, formname = null, status = null){
            axios.get('api/updatemanage-reports/'+empid+'/'+formname+'/'+status).then((response)=>{
            }).catch((err)=>{ console.log(err); });
            console.log(empid, formname, status);
        },
    },
    mounted() {

        axios.get('api/manage-reports').then((response)=>{
            this.loader = false;
            ...
            this.dtHandle = $('#reportbyUsers').DataTable({
            ...
            // Add event listener for opening and closing details
                $('#reportbyUsers tbody').on('change', 'input', function() {
                    if(rows > 0)
                    {
                        ...
                        if(this.checked) {
                            self.manageUpdateReport(row.data().empID_, $(this).val(), 1); //1 for active
                        }else{
                            self.manageUpdateReport(row.data().empID_, $(this).val(), 0); //1 for active
                        }
                    }
                });
