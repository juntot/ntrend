...
export default {
    ...
    methods:{
        updateFormByUsers(empid = null, formname = null, status = null){
            axios.get('api/updateformbyusers/'+empid+'/'+formname+'/'+status).then((response)=>{
    ...
    mounted() {
        ...
        axios.get('api/getformusers').then((response)=>{
            this.loader = false;
            ...
            this.dtHandle = $('#manageformusers').DataTable({
            ...
            // Add event listener for opening and closing details
                $('#manageformusers tbody').on('change', 'input', function() {
                    if(rows > 0)
                    {
                        ...
                        if(this.checked) {
                            self.updateFormByUsers(row.data().empID_, $(this).val(), 1); //1 for active
                        }else{
                            self.updateFormByUsers(row.data().empID_, $(this).val(), 0); //1 for active
                        }
                    }
                });
