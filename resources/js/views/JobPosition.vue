...
<template>
    <div>
        ...
        <div class="col-lg-12 canvas-chart margin-15">
            ...
            <table id="manageposition" class="mdl-data-table" style="width:100%"></table>
...
export default {
    ...
    methods:{
       addRow(val){
           this.rows.unshift({posname: val});
       },
       updateRow(val)
       {
           let row = this.$data.rows;
           row.forEach((item, index)=>{
              if(item.posID == val.posID)
              {
       ...
       setUpdate(data){
        ...
        delRow(id=''){
            ...
            if (result) {
                axios.post('api/delposition',{
                    posID: id
                }).then((res)=>{
                    if(res.data.hasOwnProperty('error')){
                        alert('Cannot delete this position. \nThere are '+res.data.error+' users from this position.\nmake sure to assign them first to another position.');
                    }else{
                        let row = this.$data.rows;
                        let data = row.filter((obj)=>obj.posID != id);
                        this.rows = data;
                    }
    ...
    mounted() {
        ...
        axios.get('api/getposition').then((response)=>{
            this.loader = false;
            this.rows=response.data;
            this.dtHandle = $('#manageposition').DataTable({
            ...
            $("#manageposition tbody").on('click', 'button.del-btn', function() {
                let tr = $(this).closest('tr');
                let row = table.row( tr );
                let dataforedit = row.data();
                // console.log(dataforedit.compID);
                self.delRow(dataforedit.posID);
                remove_emp = true;
            });

            $('#manageposition tbody').on('click', 'tr',  function(){
        ...
        //     columns: columnDefs,
        //     "sPaginationType": "simple_numbers",
        //     "dom": '<"top col-lg with-margin-bottom"f>rt<"mdl-grid"<"mdl-cell mdl-cell--4-col"i><"mdl-cell mdl-cell--8-col"p>><"clear">',
        //     "scrollX": true,
        // });
